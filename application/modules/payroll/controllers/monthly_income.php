...
// -- Purpose : 
        function ajax_list()
        {
            ...
            $output = array(                        "draw" => $_POST['draw'],                        "recordsTotal" => $this->payroll->count_all(),                        "recordsFiltered" => $this->payroll->count_filtered(),                        "data" => $data,                );
...
// -- Function Name : ajax_edit
// -- Params : $id, $period_id
// -- Purpose : 
        function ajax_edit($id, $period_id)
        {
            $master_num_rows = getAll('payroll_master', array('employee_id'=>'where/'.$id))->num_rows();
            $monthly_income_num_rows = getAll('payroll_monthly_income', array('employee_id'=>'where/'.$id, 'payroll_period_id'=>'where/'.$period_id))->num_rows();
            //print_mz($monthly_income_num_rows);
            
            if($monthly_income_num_rows>0)
            {
                $data = $this->payroll->get_by_id($id, $period_id);
            }
            else
            {
                $data = $this->payroll->get_master($id);
            }

            //print_r($this->db->last_query());
            $month = getValue('month', 'payroll_period', array('id'=>'where/'.$period_id));
            $sess_id = ($month<4) ?  date('Y')-1 :
            date('Y');
            //print_mz($sess_id);
            $monthly_income_id = getValue('id', 'payroll_monthly_income', array('employee_id'=>'where/'.$id, 'payroll_period_id'=>'where/'.$period_id));
            $master_payroll_id = getValue('id', 'payroll_master', array('employee_id'=>'where/'.$id, 'session_id'=>'where/'.$sess_id));
            //print_mz($master_payroll_id);
            ...
            {
                $data2 = $this->payroll->get_master_payroll_component($master_payroll_id)->result_array();
            }
...
// -- Purpose : 
        function ajax_update()
        {
            ...
            $data = array(                'employee_id' => $employee_id,                'payroll_group_id' => $group_id,                'payroll_period_id' => $period_id,            );
            ...
            //print_r($this->db->last_query());
            $monthly_income_id = ($num_rows>0) ? $monthly_income_id :
            $this->db->insert_id();
            $monthly_group = getAll('payroll_monthly_income', array('employee_id'=>'where/'.$employee_id, 'payroll_group_id'=>'where/'.$old_group))->num_rows();
            //print_mz($monthly_group);
            $component = array('monthly_component_id' => $this->input->post('monthly_component_id'),                           'component_id' => $this->input->post('component_id'),                           'value' => str_replace(',', '', $this->input->post('value')),                    );
            //print_mz($component);
            ...
            $data2 = array(                    'payroll_monthly_income_id'=>$monthly_income_id,                    'payroll_component_id' =>$component['component_id'][$i],                    'value' =>$component['value'][$i],                );
            
            if($component_num_rows>0)
            {
                $this->db->where('id', $component['monthly_component_id'][$i])->update('payroll_monthly_income_component', $data2);
            }
            else
            {
                $this->db->insert('payroll_monthly_income_component', $data2);
            }
...
// -- Function Name : get_component_table
// -- Params : 
// -- Purpose : 
        function get_component_table()
        {
            $id = $this->input->post('id');
            $q = $this->data['component'] = $this->payroll->get_component($id)->result();
            //lastq();print_mz($q);
...
// -- Function Name : get_periode_status
// -- Params : 
// -- Purpose : 
        function get_periode_status()
        {
            $id = $this->input->post('id');
            $status = getValue('status','payroll_period', array('id'=>'where/'.$id));
            $status = ($status == 1) ? 'Period Closed' :
            'Period Open';
            echo $status;
        }
...
// -- Function Name : print_slip
// -- Params : $id, $period_id
// -- Purpose : 
        function print_slip($id, $period_id)
        {
...
// -- Function Name : _render_page
// -- Params : $view, $data=null, $render=false
// -- Purpose : 
