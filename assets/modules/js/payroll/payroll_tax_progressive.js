...
$(document).ready(function() {
    //$(".select2").select2();
    //datatables
    table = $('#table-progressive').DataTable({ 
        ...
        "ajax": {
            "url": "payroll_tax_progressive/ajax_list/",
            "type": "POST"
        },
    ...
    /*table_progressive = $('#table-group-progressive').DataTable({
            "retrieve": true,
            "paging": false, 
            "processing": true, //Feature control the processing indicator.
            "serverSide": true, //Feature control DataTables' server-side processing mode.
            ...
            // Load data for the table's content from an Ajax source
            "ajax": {
                "url": "payroll_tax_progressive/ajax_progressive_list/",
                "type": "POST",
                "data": function ( d ) {
...
function edit_user(id)
{
    ...
    $.ajax({
        url : "payroll_tax_progressive/ajax_edit/" + id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "payroll_tax_progressive/ajax_add";
    } else {
        url = "payroll_tax_progressive/ajax_update";
    }
...
function delete_user(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "payroll_tax_progressive/ajax_delete/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
